...
fun main(args: Array<String>) {
    ...
    val floor = Plane().apply {
        ...
        material = Material(
                pattern = CheckerPattern(black, white),
                specular = 0f)
    }
...
//    }
    ...
    val middle = Sphere().apply {
        ...
        val baseColor = color(0.1f, 1f, 0.5f)
        material = Material(
                pattern = StripePattern(baseColor, baseColor * 0.8f).apply {
                    transform = scaling(0.2f, 0.2f, 0.2f) * rotationY((PI / 3).toFloat()) * rotationZ((PI / 3).toFloat())
                },
                diffuse = 0.7f,
                specular = 0.3f
        )
    ...
    val right = Sphere().apply {
        ...
        val yellow = green + red
        material = Material(
                pattern = GradientPattern(green, yellow),
                diffuse = 0.7f,
                specular = 0.3f
        )
    }
...
//                    leftWall, rightWall,
    ...
    val timeElapsed = measureTimeMillis {
        ...
        image.saveToFile("makeScenePatterns.ppm")
    }
